    ...
    <div className={cn('space-y-3 relative', className)} ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden">
          {selectedOption?.icon && (
            <div className={cn('p-2 rounded-xl shrink-0', isOpen ? 'bg-slate-100' : 'bg-white')}>
              {selectedOption.icon}
            </div>
          )}
          <span
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white rounded-[32px] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.15)] z-[300] overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top">
          {searchable && (
